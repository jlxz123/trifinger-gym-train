...
class Args:
    ...
    wandb_project_name: str = "Trifinger_ddpg"
...
def train(hydra_cfg):
    ...
    envs = create_rlgpu_env(gym_cfg=gym_cfg, cli_args=cli_args)
            ...
            if global_step % 1000 == 0:
                if args.checkpoint_path != "":
                    if torch.mean(data.rewards).item() > 50:
                        ...
                        torch.save(
                            state,
                            f"{Path(__file__).parents[3]}/trained_models/ddpg/ddpg_continus_step{global_step}_{int(torch.mean(data.rewards).item())}",
                        )
