...
@hydra.main(config_name="config", config_path=str(PATH_CFG))
def launch_rlg_hydra(cfg: DictConfig):
    log = logging.getLogger(__name__)
    OmegaConf.update(cfg, "args[play]", False)  # True为推理，False为训练（默认False）
    # 指定train和test算法
    # 支持ppo_tt, cql, sacn, sac, ddpg, td3，将下面算法修改即可
    OmegaConf.update(cfg, "train", "sac")
    OmegaConf.update(cfg, "test", "sac")
    # print(cfg.train, cfg.test)
    OmegaConf.update(cfg,"gym[task_difficulty]",1)  # 困难模式（1～4，在 leibnizgym/leibnizgym/envs/trifinger/trifinger_env.py 中查看）
    OmegaConf.update(cfg,"args[headless]",True)  # 无头模型（服务器不能可视化）
    # OmegaConf.update(cfg,"args[offline]",True)
    OmegaConf.update(cfg,"args[num_envs]",1024)  # 训练环境数目
    # OmegaConf.update(cfg,"args[checkpoint]",'/data/user/wanqiang/document/leibnizgym/models/sac_2024-05-15-21:39:25/sac_continus_step347000_837')  # 模型参数
    OmegaConf.update(cfg,"args[wandb_log]",False)  # rlgame中的wandb日志（在cleanrl中与其取反，cleanrl中会用wandb记录）
    OmegaConf.update(cfg,"gym[control_decimation]",30)  #单次动作仿真次数 
