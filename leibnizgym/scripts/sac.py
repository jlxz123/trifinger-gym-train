...
class Args:
    ...
    wandb_project_name: str = "Trifinger_sac"
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % 1000 == 0:
                if torch.mean(data.rewards).item() > 50:
                    ...
                    torch.save(
                        state,
                        f"{Path(__file__).parents[3]}/trained_models/sac/sac_continus_step{global_step}_{int(torch.mean(data.rewards).item())}",
                    )
