...
# import ddpg
# import td3
# from leibnizgym.scripts import ppo_tt, cql, sacn, sac, ddpg, td3
import sac as sac
import ppo as ppo
import m_cql as cql
...
vecenv.register(
    "RLGPU",
    lambda config_name, num_actors, **kwargs: RlGamesGpuEnvAdapter(
...
class LeibnizAlgoObserver(AlgoObserver):
    ...
    def after_init(self, algo):
        self.algo = algo
        self.game_scores = torch_ext.AverageMeter(1, self.algo.games_to_track).to(
            ...
            for ind in done_indices:
                ...
                if game_res is not None:
                    self.game_scores.update(
                        torch.from_numpy(np.asarray([game_res])).to(
                            self.algo.ppo_device
                        )
                    )
        if len(infos) > 1 and isinstance(
            infos[1], dict
...
def run_rlg_hydra(hydra_cfg):
    ...
    if cli_args.offline == False:
        ...
        if cli_args.play == True:
            eval(hydra_cfg.test).test(hydra_cfg)

        else:
            eval(hydra_cfg.train).train(hydra_cfg)
    ...
    if rl_game == 1:
        ...
        if cli_args.play == True:
            ...
            if "deterministic" in player_config:
                ...
                is_deterministic = player_config.get("deterministic", True)
            if (
                "checkpoint" in vargs
                and vargs["checkpoint"] is not None
                and vargs["checkpoint"] != ""
            ):
            ...
            for i_episode in range(1, max_episodes + 1):
                ...
                if i_episode % 200 == 0:
                    torch.save(
                        ppo.policy.state_dict(),
                        "/data/user/wanqiang/document/leibnizgym/model/PPO_continuous_{}_{}.pth".format(
                            env_name, i_episode
                        ),
                    )
                ...
                if i_episode % log_interval == 0:
                    ...
                    print(
                        "Episode {} \t Avg length: {} \t Avg reward: {}".format(
